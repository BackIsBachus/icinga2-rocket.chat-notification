...
PAYLOAD="payload={\"text\": \"${ICON} ${NOTIFICATIONTYPE} <http://${ICINGA_HOSTNAME}/icingaweb2/monitoring/host/services?host=${HOSTNAME}|${HOSTDISPLAYNAME}> (<http://${ICINGA_HOSTNAME}/icingaweb2/monitoring/service/show?h    ost=${HOSTNAME}&service=${SERVICEDESC}|${SERVICEDISPLAYNAME}>): *${SERVICESTATE}* | ${SERVICEOUTPUT} | [ *${NOTIFICATIONAUTHORNAME}* ] ${NOTIFICATIONCOMMENT} \"}"
